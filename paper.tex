...
certain angles.  This occurs because when two pieces have the same depth value,
due to perspective one of the pieces may actually appear behind the other.  In
the future, this issue may be solved by using the distance from the camera to
sort rather than the actual depth.  Figure
\ref{boolean-xor-with-artifact} shows an example of the visual artifact.
...
% Picture of depth-sorting issue
...
% approach scales as the number of samples taken increases.
