...
\ref{fragment-ray-casting} gives a depiction of the process, with green and red
circles representing fragments.  Figures \ref{fig:first-pass-of-kruger-method}
and \ref{fig:second-pass-of-kruger-method} show the first and second passes of
the Kruger method.
...
\begin{figure}
\centering
\includegraphics[width=0.8\textwidth]{back-faces.png}
\caption{First pass of Kruger method.}
\label{fig:first-pass-of-kruger-method}
\end{figure}

\begin{figure}
\centering
\includegraphics[width=0.8\textwidth]{kruger.png}
\caption{Second pass of Kruger method.}
\label{fig:second-pass-of-kruger-method}
\end{figure}
...
The XML in listing \ref{listing:xml-of-second-pass} shows how the second pass
for the depth-masking approach is rendered.  Notice that the render graph
supports basic instancing through the {\em group} and {\em instance} nodes.
When an instance node is visited, it simply traverses the group it points to.
Complex behavior can be achieved with nested groups.

\lstset{language=XML}
\lstset{markfirstintag=true}
\lstset{showstringspaces=false}
\lstset{caption=XML of second pass for depth-masking approach.}
\lstset{captionpos=b}
